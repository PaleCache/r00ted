...
(function () {
  ...
  #mpModalBackdrop {
    display: none;
    position: fixed; inset: 0; z-index: 20049;
    background: transparent;
  }
  #mpModal.show ~ #mpModalBackdrop { display: block; }

#mpWindow {
  position: fixed;
  width: 860px; max-width: 94vw;
  height: 568px; max-height: 90vh;
  top: 50%; left: 50%;
...
#mpWindow.mp-pip-mode {
  min-width: 300px;
  min-height: 200px;
}


#mpWindow.mp-pip-fullscreen {
  top: 0 !important;
  left: 0 !important;
...
#mpWindow.mp-pip-fullscreen #mpResizeHandle,
#mpWindow.mp-pip-fullscreen #mpPipResizeHandle { display: none !important; }
...
#mpWindow.mp-pip-mode #mpPipResizeHandle {
  display: block;
}
...
#mpPipFullscreenBtn:hover {
  background: #FF0000;
}

#mpWindow.mp-pip-mode #mpPipFullscreenBtn {
  display: flex;
}

#mpWindow.mp-pip-fullscreen #mpPipResizeHandle {
  display: none;
}
  ...
  #mpHeader:active { cursor: grabbing; }
...
#mpMiniToggleBtn, #mpPipToggleBtn {
    background: rgba(255,255,255,0.08); border: none; color: #b9bbbe; font-size: 14px; cursor: pointer;
    width: 30px; height: 30px; border-radius: 50%; display: flex; align-items: center; justify-content: center;
    transition: 0.15s; margin-right: 6px;
  }
  #mpMiniToggleBtn:hover, #mpPipToggleBtn:hover { background: rgba(255,255,255,0.15); color: #fff; }
  #mpPipToggleBtn.active { background: #FF0000; color: #fff; }
...
#mpVideoSpinner.show { display: block; }
...
#mpWindow.mp-pip-mode #mpBgArt,
#mpWindow.mp-pip-mode #mpBgOverlay { display: none; }

#mpWindow.mp-pip-mode #mpHeader,
#mpWindow.mp-pip-mode #mpSide,
#mpWindow.mp-pip-mode #mpTitle,
#mpWindow.mp-pip-mode #mpArtist,
#mpWindow.mp-pip-mode #mpProgressRow,
#mpWindow.mp-pip-mode #mpControls,
#mpWindow.mp-pip-mode #mpVolRow,
#mpWindow.mp-pip-mode #mpEqRow,
#mpWindow.mp-pip-mode #mpResizeHandle {
  display: none !important;
}

#mpWindow.mp-pip-mode #mpBody { padding: 0; }
#mpWindow.mp-pip-mode #mpMain { padding: 0; width: 100%; height: 100%; }

#mpWindow.mp-pip-mode #mpArtWrap {
  width: 100%;
  height: 100%;
  border-radius: 0;
  box-shadow: none;
...
#mpPipExitBtn:hover { background: #FF0000; }
#mpWindow.mp-pip-mode #mpPipExitBtn { display: flex; }

#mpPipToggleBtn.active { color: #FF0000; }
  `;
...
`;
  ...
  modal.innerHTML = `
    <div id="mpWindow">
      ...
      <div id="mpResizeHandle"></div>
      <div id="mpPipResizeHandle"></div>
      ...
      <div id="mpHeader">
        ...
        <div style="display:flex; align-items:center;">
          <button id="mpPipToggleBtn" title="Picture-in-picture mode">▣</button>
      ...
      <div id="mpBody">
        <div id="mpMain">
          <div id="mpArtWrap">
            <button id="mpPipExitBtn" title="Exit picture-in-picture">✕</button>
            <button id="mpPipFullscreenBtn" title="Fullscreen">⛶</button>
            <video id="mpArtVideo" style="display:none;" muted loop playsinline></video>
            <div id="mpVideoSpinner"></div>
  ...
  async function enableVoiceBroadcast() {
    ...
    try {
      ...
      window.showToast?.("🎵 Music bot joining please wait");
  ...
  function updateNowPlayingUI() {
    ...
    if (!track) {
      ...
      el("mpVideoSpinner").classList.remove("show");
...
if (!isSameSrc) {
  el("mpVideoSpinner").classList.add("show");
  ...
  function removeTrackAt(index) {
    ...
    if (removed) {
      if (removed.url && removed.source !== "server") {
        try { URL.revokeObjectURL(removed.url); } catch (e) {}
      }
      if (removed.artUrl && removed.artUrl.startsWith("blob:")) {
        try { URL.revokeObjectURL(removed.artUrl); } catch (e) {}
      }
      if (removed.videoUrl && removed.videoUrl.startsWith("blob:")) {
  ...
  function buildTrackRow(t, opts) {
    ...
    row.innerHTML = `
      ${libArtOk
        ? `<img class="mp-lib-thumb" loading="lazy" src="${safeArtUrl}">
  ...
  function setPipMode(enabled) {
    ...
    pipActive = enabled;
    win.classList.toggle("mp-pip-mode", enabled);
    el("mpPipToggleBtn").classList.toggle("active", enabled);
    ...
    if (enabled) {
      ...
      win.classList.remove("mp-pip-fullscreen");
    }
  }

  function setupPipDragResize() {
    const win = el("mpWindow");
    if (win.dataset.pipSetup) return;
    win.dataset.pipSetup = "true";

    let isDragging = false, isResizing = false;
    let startX, startY, startLeft, startTop, startWidth, startHeight;

    win.addEventListener("mousedown", (e) => {
      if (!win.classList.contains("mp-pip-mode")) return;
      if (win.classList.contains("mp-pip-fullscreen")) return;
      if (e.target.closest("button") || e.target.id === "mpPipResizeHandle") return;
      isDragging = true;
      const rect = win.getBoundingClientRect();
    ...
    const resizeHandle = el("mpPipResizeHandle");
    if (resizeHandle) {
   ...
   document.addEventListener("mousemove", (e) => {
      if (isDragging && win.classList.contains("mp-pip-mode")) {
      ...
      if (isResizing && win.classList.contains("mp-pip-mode")) {
        const rect = win.getBoundingClientRect();
        const maxWidth = Math.max(300, window.innerWidth - rect.left);
        const maxHeight = Math.max(200, window.innerHeight - rect.top);
        const w = Math.max(300, Math.min(startWidth + (e.clientX - startX), maxWidth));
        const h = Math.max(200, Math.min(startHeight + (e.clientY - startY), maxHeight));
  ...
  function setupMainWindowDragResize() {
    const win = el("mpWindow");
    const header = el("mpHeader");
    const resizeHandle = el("mpResizeHandle");
    ...
    document.addEventListener("mousemove", (e) => {
      ...
      if (isResizing && !win.classList.contains("mp-pip-mode")) {
        win.style.width  = Math.max(520, startWidth  + (e.clientX - startX)) + "px";
        win.style.height = Math.max(380, startHeight + (e.clientY - startY)) + "px";
      }
    });
  ...
  function openPlayer() {
    ...
    el("mpWindow").classList.add("show");
  }

  function closePlayer() {
    modal.classList.remove("show");
    if (!el("mpWindow").classList.contains("mp-pip-mode")) {
      el("mpWindow").classList.remove("show");
    }
  }
  ...
  function wireEvents() {
    ...
    el("mpCloseBtn").addEventListener("click", closePlayer);
    ...
    audio.addEventListener("timeupdate", () => {
      ...
      const videoEl = el("mpArtVideo");
      const drift = videoEl.currentTime - audio.currentTime;
      if (Math.abs(drift) > 0.75) {
        videoEl.currentTime = audio.currentTime;
        videoEl.playbackRate = 1;
      } else if (Math.abs(drift) > 0.08) {
        videoEl.playbackRate = drift > 0 ? 0.97 : 1.03;
      } else {
        videoEl.playbackRate = 1;
      }
    });
    ...
    document.addEventListener("keydown", (e) => {
      if (!modal.classList.contains("show") && !el("mpWindow").classList.contains("show")) return;
      if (e.key === "Escape" && el("mpWindow").classList.contains("mp-pip-mode")) {
        e.stopPropagation();
        setPipMode(false);
        return;
      }
    ...
    setupMainWindowDragResize();
    el("mpPipToggleBtn").addEventListener("click", () => {
      setPipMode(!el("mpWindow").classList.contains("mp-pip-mode"));
    });

    el("mpPipFullscreenBtn").addEventListener("click", (e) => {
      e.stopPropagation();
      const win = el("mpWindow");
      const goingFullscreen = !win.classList.contains("mp-pip-fullscreen");
      ...
      if (goingFullscreen) {
        ...
        win.style.transform = "none";
        win.classList.add("mp-pip-fullscreen");
      } else {
        win.classList.remove("mp-pip-fullscreen");
    ...
    el("mpPipExitBtn").addEventListener("click", (e) => {
      e.stopPropagation();
      setPipMode(false);
    });
  }
